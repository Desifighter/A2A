...
export const searchMovies = ai.defineTool(
  {
    name: "searchMovies",
    description: "search OMDb for movies by title",
    inputSchema: z.object({
      query: z.string(),
    }),
  },
  async ({ query }) => {
    console.log("[omdb:searchMovies]", JSON.stringify(query));
    try {
...
export const searchPeople = ai.defineTool(
  {
    name: "searchPeople",
    description: "search OMDb for people by name (fallback to title search)",
    inputSchema: z.object({
      query: z.string(),
    }),
  },
  async ({ query }) => {
    console.log("[omdb:searchPeople]", JSON.stringify(query));
    try {
      ...
      const results = data.results.map((item: any) => ({
        name: item.Title,
        known_for: [],
        profile_path: item.Poster !== "N/A" ? item.Poster : null,
      }));
